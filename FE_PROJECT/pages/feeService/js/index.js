...
$(function() {
    ...
    function getProductList() {
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                method: "com.appwl.service.tel.products",
                partyId: "1",
            ...
            success(res) {
                ...
                if (res.resultCode == '00000') {
                    ...
                    res.data.prodList.forEach((item, index) => {
                        ...
                        //     classArr += ' recommend';
                        // } 话费没有推荐
                        str += `<div class="${classArr}" data-prodId="${item.prodId}">
                            <em>${item.prodName}</em> <span>售价${item.price}元</span>  
                            </div>`
                    })
    ...
    function getDiscountList(id) {
        ...
        $.ajax({
            ...
            headers: { 'Content-Type': 'application/json' },
            data: JSON.stringify({
                method: "com.appwl.service.tel.order",
                partyId: "1",
                time: "20171018152708897",
                v: "1.0",
                userIdentity: curUserIdentity,
    ...
    function payOrder() {
        $('#js_pay_way ul li').off('click').on('click', function() {
            ...
            $.ajax({
                ...
                headers: { 'Content-Type': 'application/json' },
                data: JSON.stringify({
                    "method": "com.appwl.service.tel.order.pay",
                    "partyId": "1",
                    "time": "20171018152708897",
                    "v": "1.0",
                    "userIdentity": curUserIdentity,
